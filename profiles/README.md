# Summary
При помощи pprof удалось найти, что в compress/gzip инициализировались writer и reader на каждый запрос. 
Заменено постоянное создание новых объектов `gzip.Writer` и `gzip.Reader` на переиспользование через `sync.Pool`

### Ключевые результаты
- Снижение аллокаций gzip writer'ов на 94% - с 231.82MB до 14.10MB
- Общее уменьшение потребления памяти на 83% - с 354.11MB до 61.21MB
